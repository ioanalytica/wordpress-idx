...
class WordpressIdxSearch {
	...
	public function __construct() {
		add_filter( 'the_content', array( $this, 'replace_marker' ) );
...
<div id="idx-search" class="idx-search">
	...
	public function register_settings() {
		...
		add_filter(
			'option_page_capability_idx_search_settings',
			function () {
				return 'edit_posts';
			}
		);
	...
	public function enqueue_admin_assets( $hook ) {
		if ( 'settings_page_idx-search' !== $hook ) {
			return;
		}
		$t = $this->i18n();
		wp_enqueue_script(
			...
			<form method="post" action="options.php">
				...
				<table class="form-table">
					<tr>
						...
						<td>
							<select name="<?php echo esc_attr( self::OPTION_LANG ); ?>" id="idx_lang">
					...
					<tr>
						...
						<td>
							<input type="text" name="<?php echo esc_attr( self::OPTION_API_KEY ); ?>" id="idx_api_key"
									value="<?php echo esc_attr( $api_key ); ?>" class="regular-text"
									autocomplete="off"<?php disabled( $is_admin, false ); ?>>
